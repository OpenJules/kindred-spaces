...
import { Asset, GeneratedMockup, AppView, LoadingState, PlacedLayer, ModelType } from './types';
...
const AssetSection = ({ 
  ...
  const handleGenerate = async () => {
    ...
    // If pro model is selected, validate API key
    if (modelType === 'pro' && !(await validateApiKey())) return;

    setIsGenerating(true);
    try {
      const b64 = await generateAsset(genPrompt, type, modelType);
      onAdd({
...
export default function App() {
  ...
  const [modelType, setModelType] = useState<ModelType>('free');

  // API Key Management
  const { showApiKeyDialog, setShowApiKeyDialog, hasKey, checkKey, openKeySelector } = useApiKey();
  ...
  const handleGenerate = async () => {
    ...
    // If pro model is selected, validate API key
    if (modelType === 'pro' && !(await checkKey())) {
      setShowApiKeyDialog(true);
      return;
    }
    ...
    try {
      const resultImage = await generateMockup(product, layers, currentPrompt, modelType);
           ...
           <div className="flex items-center gap-4">
              <div className="flex items-center bg-zinc-900 rounded-lg p-1 border border-zinc-800">
                <button 
                  ...
                  className={`px-3 py-1 text-xs font-bold rounded-md transition-all ${modelType === 'free' ? 'bg-zinc-800 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}
                >
                  FREE
                </button>
                <button 
                  ...
                  className={`px-3 py-1 text-xs font-bold rounded-md transition-all flex items-center gap-1 ${modelType === 'pro' ? 'bg-indigo-600 text-white shadow-[0_0_10px_rgba(79,70,229,0.4)]' : 'text-zinc-500 hover:text-zinc-300'}`}
                >
                  PRO <Zap size={12} className={modelType === 'pro' ? 'fill-white' : ''} />
                </button>
              </div>
